...
public class ThuThuDAO implements DAO<ThuThu> {
    ...
    public ThuThu getById(String id) {
        ...
        try (SQLiteDatabase db = helper.getReadableDatabase();
            ...
            if (cs != null && cs.getCount() > 0) {
                    ...
                    String ten = cs.getString(2);
                    String sdt = cs.getString(3);
                    return new ThuThu(id, password, ten, sdt);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @Override
    public long insert(ThuThu thuThu) {ContentValues contentValues = new ContentValues();
    ...
    public long delete(ThuThu thuThu) {
        try (SQLiteDatabase db = helper.getWritableDatabase()) {
            return db.delete(DBHelper.TABLE_THU_THU
                    , DBHelper.THU_THU_ID + " = ?"
                    , new String[] {thuThu.getMaThuThu()});
        }
    }
